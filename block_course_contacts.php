...
class block_course_contacts extends block_base
{
     ...
     * @param bool    $parent            (optional) if true, get list of users assigned in higher context too
     * @param string  $fields            (optional) fields from user (u.) , role assignment (ra) or role (r.)
     * @param string  $sort              (optional) sort from user (u.) , role assignment (ra) or role (r.)
     * @param bool    $gethiddenignored (optional) use enrolments instead
     * @param string  $group             (optional) defaults to ''
     * @param mixed   $limitfrom         (optional) defaults to ''
     * @param mixed   $limitnum          (optional) defaults to ''
     * @param string  $extrawheretest    (optional) defaults to ''
    ...
    public function get_content() {
        ...
        $userfields = array('lastaccess', 'firstname', 'lastname', 'email', 'phone1', 'picture', 'imagealt');
        $userfields = \user_picture::fields('u', $userfields);
                    ...
                    foreach ($contacts as $contact) {
                        ...
                        if ($USER->id != $contact->id) {
                            ...
                            if ($this->config->email == 1) {
                                ...
                                $content .= html_writer::link($url, $img, array('target' => '_blank'));
                            }
                            ...
                            if ($this->config->phone == 1 && $contact->phone1 != "") {
                                ...
                                $content .= html_writer::link($url, $img, array());
                            }
                        }
